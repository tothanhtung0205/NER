...
class regex:
    def __init__(self):
        ...
        self.normalize_dot = re.compile(u'\.+')
        self.detect_url = re.compile(u'(https|http|ftp|ssh)://[^\s]+', re.I)
        self.detect_num = re.compile(u'(\d+\s*,\s*\d+\w*)|(\d+\s*\.\s*\d+\w*)|(\d+\w*)')
    ...
    def run_regex_training(self, data):
        ...
        # s = self.re.detect_acronym.sub(r'3', s) # replace acronym to 3
    ...
    def detect_non_vietnamese(self):
        ...
        return re.compile(ur'\w*(' + s + u')\w*', re.I)
